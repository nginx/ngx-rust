...
fn main() -> Result<(), BoxError> {
    ...
    let nginx = if cfg!(all(unix, feature = "unittest")) {
        NginxSource::from_vendored()
    } else {
        NginxSource::from_env()
    };
...
fn generate_binding(nginx: &NginxSource) {
    ...
    let ParsedMakefile {
        ...
        defines,
        libs,
    } = parse_makefile(&autoconf_makefile_path);
    ...
    let libs: Vec<_> = libs
        .into_iter()
        .map(|lib_opt| {
            if let LibOption::LibPath(path) = lib_opt {
                if Path::new(&path).is_absolute() {
                    LibOption::LibPath(path)
                } else {
                    LibOption::LibPath(nginx.source_dir.join(path).to_string_lossy().to_string())
                }
            } else {
                lib_opt
            }
    ...
    print_cargo_metadata(nginx, &includes, &defines, &libs).expect("cargo dependency metadata");
...
/// Represents a library flags for NGINX:
/// * `LibPath`: A library directory path
/// * `Lib`: A library name
pub enum LibOption {
    LibPath(String),
    Lib(String),
...
/// Parsed representation of the NGINX Makefile:
/// * `includes`: List of include paths
/// * `defines`: List of preprocessor definitions
/// * `libs`: List of libraries and/or library directories
pub struct ParsedMakefile {
    pub includes: Vec<PathBuf>,
    pub defines: Vec<(String, Option<String>)>,
    pub libs: Vec<LibOption>,
}
...
pub fn parse_makefile(nginx_autoconf_makefile_path: &PathBuf) -> ParsedMakefile {
    fn parse_line(
    ...
    fn parse_lib_line(libs: &mut Vec<LibOption>, line: &str) {
        let mut words = shlex::Shlex::new(line);
    ...
    for l in lines {
        ...
        } else if let Some(tail) = line.strip_prefix("LIBNGINX_LDFLAGS") {
            parse_lib_line(&mut libs, tail);
        }
    ...
    ParsedMakefile {
        includes: cflags_includes.into_iter().map(PathBuf::from).collect(),
        defines,
        libs,
    }
}
...
pub fn print_cargo_metadata<T: AsRef<Path>>(
    ...
    {
        // Linker flags to use libnginx in tests
        println!(
        ...
        for lib in _libs {
            if let LibOption::LibPath(ref path) = lib {
                println!("cargo::rustc-link-search={path}");
            } else if let LibOption::Lib(ref name) = lib {
